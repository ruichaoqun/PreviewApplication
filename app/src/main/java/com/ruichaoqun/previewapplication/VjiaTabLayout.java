 ...
 *
 * <p>Population of the tabs to display is done through {@link VjiaTabLayout.Tab} instances. You create tabs via
 * {@link #newTab()}. From there you can change the tab's label or icon via {@link VjiaTabLayout.Tab#setText(int)}
 * and {@link VjiaTabLayout.Tab#setIcon(int)} respectively. To display the tab, you need to add it to the layout
 * via one of the {@link #addTab(VjiaTabLayout.Tab)} methods. For example:
 *
 * <pre>
 * TabLayout tabLayout = ...;
 ...
 * </pre>
 *
 * You should add a listener via {@link #addOnTabSelectedListener(VjiaTabLayout.OnTabSelectedListener)} to be
 ...
 *
 */
public class VjiaTabLayout extends HorizontalScrollView {
    ...
    private static final Pools.Pool<VjiaTabLayout.Tab> tabPool = new Pools.SynchronizedPool<>(16);
    ...
    /**
     * If a tab is instantiated with {@link VjiaTabLayout.Tab#setText(CharSequence)}, and this mode is set, the text
     * will be saved and utilized for the content description, but no visible labels will be created.
     *
     * @see VjiaTabLayout.Tab#setTabLabelVisibility(int)
     */
    public static final int TAB_LABEL_VISIBILITY_UNLABELED = 0;

    /**
     * This mode is set by default. If a tab is instantiated with {@link VjiaTabLayout.Tab#setText(CharSequence)}, a
    ...
    /**
     * Gravity used to fill the {@link VjiaTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link VjiaTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
     ...
     * VjiaTabLayout}. This will only take effect if the indicator height is set via the custom indicator
     ...
     * width of the {@link VjiaTabLayout}. This will disregard {@code tabIndicatorHeight} and the
    ...
    public interface OnTabSelectedListener extends VjiaTabLayout.BaseOnTabSelectedListener<VjiaTabLayout.Tab> {
    }

    /** Callback interface invoked when a tab's selection state changes. */
    @Deprecated
    public interface BaseOnTabSelectedListener<T extends VjiaTabLayout.Tab> {
        /**
    ...
    private final ArrayList<VjiaTabLayout.Tab> tabs = new ArrayList<>();
    @Nullable private VjiaTabLayout.Tab selectedTab;

    private final RectF tabViewContentBounds = new RectF();

    @NonNull private final VjiaTabLayout.SlidingTabIndicator slidingTabIndicator;
    ...
    @VjiaTabLayout.TabIndicatorGravity
    ...
    @Nullable private VjiaTabLayout.BaseOnTabSelectedListener selectedListener;

    private final ArrayList<VjiaTabLayout.BaseOnTabSelectedListener> selectedListeners = new ArrayList<>();
    @Nullable private VjiaTabLayout.BaseOnTabSelectedListener currentVpSelectedListener;
    ...
    private VjiaTabLayout.TabLayoutOnPageChangeListener pageChangeListener;
    private VjiaTabLayout.AdapterChangeListener adapterChangeListener;
    private boolean setupViewPagerImplicitly;

    // Pool we use as a simple RecyclerBin
    private final Pools.Pool<VjiaTabLayout.TabView> tabViewPool = new Pools.SimplePool<>(12);

    public VjiaTabLayout(@NonNull Context context) {
        this(context, null);
    }

    public VjiaTabLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.tabStyle);
    }

    @SuppressLint("RestrictedApi")
    public VjiaTabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        // Add the TabStrip
        slidingTabIndicator = new VjiaTabLayout.SlidingTabIndicator(context);
        super.addView(
                slidingTabIndicator,
                0,
        ...
        TypedArray a =
                ThemeEnforcement.obtainStyledAttributes(
                        context,
                        attrs,
                        R.styleable.VjiaTabLayout,
                        defStyleAttr,
                        R.style.Widget_Design_TabLayout,
                        R.styleable.VjiaTabLayout_tabTextAppearance);
        ...
        slidingTabIndicator.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabIndicatorHeight, -1));
        slidingTabIndicator.setSelectedIndicatorColor(
                a.getColor(R.styleable.VjiaTabLayout_tabIndicatorColor, 0));
        setSelectedTabIndicator(
                MaterialResources.getDrawable(context, a, R.styleable.VjiaTabLayout_tabIndicator));
        setSelectedTabIndicatorGravity(
                a.getInt(R.styleable.VjiaTabLayout_tabIndicator_Gravity, INDICATOR_GRAVITY_BOTTOM));
        setTabIndicatorFullWidth(a.getBoolean(R.styleable.VjiaTabLayout_tabIndicatorFullWidth, true));

        tabPaddingStart =
                tabPaddingTop =
                        tabPaddingEnd =
                                tabPaddingBottom = a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabPadding, 0);
        tabPaddingStart =
                a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabPaddingStart, tabPaddingStart);
        tabPaddingTop = a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabPaddingTop, tabPaddingTop);
        tabPaddingEnd = a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabPaddingEnd, tabPaddingEnd);
        tabPaddingBottom =
                a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabPaddingBottom, tabPaddingBottom);

        tabIndicatorMargin = a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabIndicatorMargin,0);
        tabTextAppearance =
                a.getResourceId(R.styleable.VjiaTabLayout_tabTextAppearance, R.style.TextAppearance_Design_Tab);
        ...
        if (a.hasValue(R.styleable.VjiaTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            tabTextColors =
                    MaterialResources.getColorStateList(context, a, R.styleable.VjiaTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.VjiaTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.VjiaTabLayout_tabSelectedTextColor, 0);
            tabTextColors = createColorStateList(tabTextColors.getDefaultColor(), selected);
        }

        tabIconTint =
                MaterialResources.getColorStateList(context, a, R.styleable.VjiaTabLayout_tabIconTint);
        tabIconTintMode =
                ViewUtils.parseTintMode(a.getInt(R.styleable.VjiaTabLayout_tabIcon_TintMode, -1), null);

        tabRippleColorStateList =
                MaterialResources.getColorStateList(context, a, R.styleable.VjiaTabLayout_tabRippleColor);

        tabIndicatorAnimationDuration =
                a.getInt(R.styleable.VjiaTabLayout_tabIndicatorAnimationDuration, ANIMATION_DURATION);

        requestedTabMinWidth =
                a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabMinWidth, INVALID_WIDTH);
        requestedTabMaxWidth =
                a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabMaxWidth, INVALID_WIDTH);
        tabBackgroundResId = a.getResourceId(R.styleable.VjiaTabLayout_tabBackground, 0);
        contentInsetStart = a.getDimensionPixelSize(R.styleable.VjiaTabLayout_tabContentStart, 0);
        // noinspection WrongConstant
        mode = a.getInt(R.styleable.VjiaTabLayout_tab_Mode, MODE_FIXED);
        tabGravity = a.getInt(R.styleable.VjiaTabLayout_tab_Gravity, GRAVITY_FILL);
        inlineLabel = a.getBoolean(R.styleable.VjiaTabLayout_tabInlineLabel, false);
        unboundedRipple = a.getBoolean(R.styleable.VjiaTabLayout_tabUnboundedRipple, false);
        a.recycle();
     ...
     * @attr ref com.google.android.material.R.styleable#VjiaTabLayout_tabIndicatorColor
     */
     ...
     * @attr ref com.google.android.material.R.styleable#VjiaTabLayout_tabIndicatorHeight
     */
    @Deprecated
     ...
     */
    public void addTab(@NonNull VjiaTabLayout.Tab tab) {
     ...
     */
    public void addTab(@NonNull VjiaTabLayout.Tab tab, int position) {
     ...
     */
    public void addTab(@NonNull VjiaTabLayout.Tab tab, boolean setSelected) {
     ...
     * @param setSelected True if the added tab should become the selected tab.
     */
    public void addTab(@NonNull VjiaTabLayout.Tab tab, int position, boolean setSelected) {
        if (tab.parent != this) {
    ...
    private void addTabFromItemView(@NonNull TabItem item) {
        final VjiaTabLayout.Tab tab = newTab();
        if (item.text != null) {
    ...
    /**
     * @deprecated Use {@link #addOnTabSelectedListener(VjiaTabLayout.OnTabSelectedListener)} and {@link
     *    #removeOnTabSelectedListener(VjiaTabLayout.OnTabSelectedListener)}.
     */
    @Deprecated
    public void setOnTabSelectedListener(@Nullable VjiaTabLayout.OnTabSelectedListener listener) {
        setOnTabSelectedListener((VjiaTabLayout.BaseOnTabSelectedListener) listener);
    }
     ...
     */
    @Deprecated
    public void setOnTabSelectedListener(@Nullable VjiaTabLayout.BaseOnTabSelectedListener listener) {
    ...
    /**
     * Add a {@link VjiaTabLayout.OnTabSelectedListener} that will be invoked when tab selection changes.
     *
     ...
     */
    public void addOnTabSelectedListener(@NonNull VjiaTabLayout.OnTabSelectedListener listener) {
        addOnTabSelectedListener((VjiaTabLayout.BaseOnTabSelectedListener) listener);
    }

    /**
     * Add a {@link VjiaTabLayout.BaseOnTabSelectedListener} that will be invoked when tab selection
     * changes.
     *
     * <p>Components that add a listener should take care to remove it when finished via {@link
     * #removeOnTabSelectedListener(VjiaTabLayout.BaseOnTabSelectedListener)}.
     *
     * @param listener listener to add
     * @deprecated use {@link #addOnTabSelectedListener(VjiaTabLayout.OnTabSelectedListener)}
     */
    @Deprecated
    public void addOnTabSelectedListener(@Nullable VjiaTabLayout.BaseOnTabSelectedListener listener) {
    ...
    /**
     * Remove the given {@link VjiaTabLayout.OnTabSelectedListener} that was previously added via {@link
     * #addOnTabSelectedListener(VjiaTabLayout.OnTabSelectedListener)}.
     *
     * @param listener listener to remove
     */
    public void removeOnTabSelectedListener(@NonNull VjiaTabLayout.OnTabSelectedListener listener) {
        removeOnTabSelectedListener((VjiaTabLayout.BaseOnTabSelectedListener) listener);
    }

    /**
     * Remove the given {@link VjiaTabLayout.BaseOnTabSelectedListener} that was previously added via
     * {@link #addOnTabSelectedListener(VjiaTabLayout.BaseOnTabSelectedListener)}.
     *
     * @param listener listener to remove
     * @deprecated use {@link #removeOnTabSelectedListener(VjiaTabLayout.OnTabSelectedListener)}
     */
    @Deprecated
    public void removeOnTabSelectedListener(@Nullable VjiaTabLayout.BaseOnTabSelectedListener listener) {
        selectedListeners.remove(listener);
    }

    /** Remove all previously added {@link VjiaTabLayout.OnTabSelectedListener}s. */
    public void clearOnTabSelectedListeners() {
        selectedListeners.clear();
    }

    /**
     * Create and return a new {@link VjiaTabLayout.Tab}. You need to manually add this using {@link #addTab(VjiaTabLayout.Tab)}
     * or a related method.
     *
     * @return A new Tab
     * @see #addTab(VjiaTabLayout.Tab)
     */
    @NonNull
    public VjiaTabLayout.Tab newTab() {
        VjiaTabLayout.Tab tab = createTabFromPool();
        tab.parent = this;
    ...
    protected VjiaTabLayout.Tab createTabFromPool() {
        VjiaTabLayout.Tab tab = tabPool.acquire();
        if (tab == null) {
            tab = new VjiaTabLayout.Tab();
        }
        return tab;
    }

    // TODO(b/76413401): remove this method and just create the final field after the widget migration
    protected boolean releaseFromTabPool(VjiaTabLayout.Tab tab) {
    ...
    @Nullable
    public VjiaTabLayout.Tab getTabAt(int index) {
     ...
     */
    public void removeTab(@NonNull VjiaTabLayout.Tab tab) {
        if (tab.parent != this) {
    ...
    public void removeTabAt(int position) {
        ...
        final VjiaTabLayout.Tab removedTab = tabs.remove(position);
        if (removedTab != null) {
            removedTab.reset();
    ...
    public void removeAllTabs() {
        ...
        for (final Iterator<VjiaTabLayout.Tab> i = tabs.iterator(); i.hasNext(); ) {
            final VjiaTabLayout.Tab tab = i.next();
            i.remove();
            tab.reset();
     ...
     */
    public void setTabMode(@VjiaTabLayout.Mode int mode) {
        if (mode != this.mode) {
            this.mode = mode;
            applyModeAndGravity();
        }
    ...
    /**
     * Returns the current mode used by this {@link VjiaTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @VjiaTabLayout.Mode
     ...
     */
    public void setTabGravity(@VjiaTabLayout.TabGravity int gravity) {
     ...
     */
    @VjiaTabLayout.TabGravity
    public int getTabGravity() {
        return tabGravity;
    }

    /**
     * Set the indicator gravity used to align the tab selection indicator in the {@link VjiaTabLayout}.
     ...
     * indicator height and stretch across the entire height and width of the {@link VjiaTabLayout}. This
     ...
     */
    public void setSelectedTabIndicatorGravity(@VjiaTabLayout.TabIndicatorGravity int indicatorGravity) {
    ...
    public void setInlineLabel(boolean inline) {
        if (inlineLabel != inline) {
            ...
            for (int i = 0; i < slidingTabIndicator.getChildCount(); i++) {
                ...
                if (child instanceof VjiaTabLayout.TabView) {
                    ((VjiaTabLayout.TabView) child).updateOrientation();
                }
            }
            applyModeAndGravity();
        }
    ...
    /**
     * Set whether this {@link VjiaTabLayout} will have an unbounded ripple effect or if ripple will be
    ...
    public void setUnboundedRipple(boolean unboundedRipple) {
        if (this.unboundedRipple != unboundedRipple) {
            ...
            for (int i = 0; i < slidingTabIndicator.getChildCount(); i++) {
                View child = slidingTabIndicator.getChildAt(i);
                if (child instanceof VjiaTabLayout.TabView) {
                    ((VjiaTabLayout.TabView) child).updateBackgroundDrawable(getContext());
                }
            }
        }
    }
    ...
    /**
     * Returns whether this {@link VjiaTabLayout} has an unbounded ripple effect, or if ripple is bound to
     * the tab item size.
     *
    ...
    /**
     * The one-stop shop for setting up this {@link VjiaTabLayout} with a {@link ViewPager}.
     *
    ...
    private void setupWithViewPager(
        ...
        if (viewPager != null) {
            ...
            if (pageChangeListener == null) {
                pageChangeListener = new VjiaTabLayout.TabLayoutOnPageChangeListener(this);
            }
            pageChangeListener.reset();
            viewPager.addOnPageChangeListener(pageChangeListener);

            // Now we'll add a tab selected listener to set ViewPager's current item
            currentVpSelectedListener = new VjiaTabLayout.ViewPagerOnTabSelectedListener(viewPager);
            ...
            if (adapterChangeListener == null) {
                adapterChangeListener = new VjiaTabLayout.AdapterChangeListener();
            }
    ...
    void setPagerAdapter(@Nullable final PagerAdapter adapter, final boolean addObserver) {
        ...
        if (addObserver && adapter != null) {
            ...
            if (pagerAdapterObserver == null) {
                pagerAdapterObserver = new VjiaTabLayout.PagerAdapterObserver();
            }
    ...
    @NonNull
    private VjiaTabLayout.TabView createTabView(@NonNull final VjiaTabLayout.Tab tab) {
        VjiaTabLayout.TabView tabView = tabViewPool != null ? tabViewPool.acquire() : null;
        if (tabView == null) {
            tabView = new VjiaTabLayout.TabView(getContext());
        }
        tabView.setTab(tab);
    ...
    private void configureTab(@NonNull VjiaTabLayout.Tab tab, int position) {
    ...
    private void addTabView(@NonNull VjiaTabLayout.Tab tab) {
        final VjiaTabLayout.TabView tabView = tab.view;
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        for (int i = 0; i < slidingTabIndicator.getChildCount(); i++) {
            ...
            if (tabView instanceof VjiaTabLayout.TabView) {
                ((VjiaTabLayout.TabView) tabView).drawBackground(canvas);
            }
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // height
        @SuppressLint("RestrictedApi") final int idealHeight = (int) ViewUtils.dpToPx(getContext(), getDefaultHeight());
    ...
    private void removeTabViewAt(int position) {
        final VjiaTabLayout.TabView view = (VjiaTabLayout.TabView) slidingTabIndicator.getChildAt(position);
    ...
    private void animateToTab(int newPosition) {
        if (newPosition == VjiaTabLayout.Tab.INVALID_POSITION) {
            return;
        }
    ...
    private void setSelectedTabView(int position) {
        ...
        if (position < tabCount) {
            for (int i = 0; i < tabCount; i++) {
                ...
                //TODO 更改选中文字大小
                if(i == position){
                    child.animate().scaleX(1.2f).scaleY(1.2f).setDuration(200).start();
                }else{
     ...
     * @see #selectTab(VjiaTabLayout.Tab, boolean)
     */
    public void selectTab(@Nullable VjiaTabLayout.Tab tab) {
        selectTab(tab, true);
    }
     ...
     * @see #selectTab(VjiaTabLayout.Tab)
     */
    public void selectTab(@Nullable final VjiaTabLayout.Tab tab, boolean updateIndicator) {
        final VjiaTabLayout.Tab currentTab = selectedTab;
        ...
        if (currentTab == tab) {
            ...
            final int newPosition = tab != null ? tab.getPosition() : VjiaTabLayout.Tab.INVALID_POSITION;
            if (updateIndicator) {
                if ((currentTab == null || currentTab.getPosition() == VjiaTabLayout.Tab.INVALID_POSITION)
                        && newPosition != VjiaTabLayout.Tab.INVALID_POSITION) {
    ...
    private void dispatchTabSelected(@NonNull final VjiaTabLayout.Tab tab) {
        for (int i = selectedListeners.size() - 1; i >= 0; i--) {
            selectedListeners.get(i).onTabSelected(tab);
        }
    }

    private void dispatchTabUnselected(@NonNull final VjiaTabLayout.Tab tab) {
        for (int i = selectedListeners.size() - 1; i >= 0; i--) {
            selectedListeners.get(i).onTabUnselected(tab);
        }
    }

    private void dispatchTabReselected(@NonNull final VjiaTabLayout.Tab tab) {
        for (int i = selectedListeners.size() - 1; i >= 0; i--) {
    ...
    public static class Tab {
        ...
        private @VjiaTabLayout.LabelVisibility
        ...
        @Nullable public VjiaTabLayout parent;
        // TODO(b/76413401): make package private after the widget migration is finished
        @NonNull public VjiaTabLayout.TabView view;
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setTag(@Nullable Object tag) {
            this.tag = tag;
            return this;
        }
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setCustomView(@Nullable View view) {
            customView = view;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setCustomView(@LayoutRes int resId) {
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setIcon(@Nullable Drawable icon) {
            this.icon = icon;
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setIcon(@DrawableRes int resId) {
            if (parent == null) {
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setText(@Nullable CharSequence text) {
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setText(@StringRes int resId) {
            if (parent == null) {
                throw new IllegalArgumentException("Tab not attached to a TabLayout");
            }
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setTabLabelVisibility(@VjiaTabLayout.LabelVisibility int mode) {
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setContentDescription(@StringRes int resId) {
            if (parent == null) {
                throw new IllegalArgumentException("Tab not attached to a TabLayout");
            }
         ...
         */
        @NonNull
        public VjiaTabLayout.Tab setContentDescription(@Nullable CharSequence contentDesc) {
    ...
    /** A {@link LinearLayout} containing {@link VjiaTabLayout.Tab} instances for use with {@link VjiaTabLayout}. */
    public final class TabView extends LinearLayout {
        private VjiaTabLayout.Tab tab;
        ...
        protected void drawableStateChanged() {
            ...
            if (changed) {
                invalidate();
                VjiaTabLayout.this.invalidate(); // Invalidate TabLayout, which draws mBaseBackgroundDrawable
            }
        }
        ...
        void setTab(@Nullable final VjiaTabLayout.Tab tab) {
            if (tab != this.tab) {
                this.tab = tab;
                update();
            }
        ...
        final void update() {
            final VjiaTabLayout.Tab tab = this.tab;
        ...
        @Nullable
        public VjiaTabLayout.Tab getTab() {
            return tab;
        }
    ...
    private class SlidingTabIndicator extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (!tabIndicatorFullWidth && selectedTitle instanceof VjiaTabLayout.TabView) {
                    calculateTabViewContentBounds((VjiaTabLayout.TabView) selectedTitle, tabViewContentBounds);
                ...
                if (selectionOffset > 0f && selectedPosition < getChildCount() - 1) {
                    ...
                    if (!tabIndicatorFullWidth && nextTitle instanceof VjiaTabLayout.TabView) {
                        calculateTabViewContentBounds((VjiaTabLayout.TabView) nextTitle, tabViewContentBounds);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (!tabIndicatorFullWidth && targetView instanceof VjiaTabLayout.TabView) {
                calculateTabViewContentBounds((VjiaTabLayout.TabView) targetView, tabViewContentBounds);
        ...
        /**
         * Given a {@link VjiaTabLayout.TabView}, calculate the left and right bounds of its content.
         *
         ...
         *
         * @param tabView {@link VjiaTabLayout.TabView} for which to calculate left and right content bounds.
         */
        private void calculateTabViewContentBounds(
                @NonNull VjiaTabLayout.TabView tabView, @NonNull RectF contentBounds) {
        ...
        public void draw(@NonNull Canvas canvas) {
            ...
            if(tabIndicatorMargin != 0){
                indicatorTop -=  tabIndicatorMargin;
                indicatorBottom -= tabIndicatorMargin;
            }
...
//                        DrawableCompat.setTint(selectedIndicator, selectedIndicatorPaint.getColor());
    ...
    private int getDefaultHeight() {
        ...
        for (int i = 0, count = tabs.size(); i < count; i++) {
            VjiaTabLayout.Tab tab = tabs.get(i);
     ...
     * provided {@link VjiaTabLayout} so that the tab position is kept in sync.
     *
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        @NonNull private final WeakReference<VjiaTabLayout> tabLayoutRef;
        private int previousScrollState;
        private int scrollState;

        public TabLayoutOnPageChangeListener(VjiaTabLayout vjiaTabLayout) {
            tabLayoutRef = new WeakReference<>(vjiaTabLayout);
        }
        ...
        public void onPageScrolled(
            ...
            final VjiaTabLayout vjiaTabLayout = tabLayoutRef.get();
            if (vjiaTabLayout != null) {
                ...
                vjiaTabLayout.setScrollPosition(position, positionOffset, updateText, updateIndicator);
            }
        }
        ...
        public void onPageSelected(final int position) {
            final VjiaTabLayout vjiaTabLayout = tabLayoutRef.get();
            if (vjiaTabLayout != null
                    && vjiaTabLayout.getSelectedTabPosition() != position
                    && position < vjiaTabLayout.getTabCount()) {
                ...
                vjiaTabLayout.selectTab(vjiaTabLayout.getTabAt(position), updateIndicator);
            }
        }
    ...
    /**
     * A {@link VjiaTabLayout.OnTabSelectedListener} class which contains the necessary calls back to the
     * provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements VjiaTabLayout.OnTabSelectedListener {
        ...
        @Override
        public void onTabSelected(@NonNull VjiaTabLayout.Tab tab) {
            viewPager.setCurrentItem(tab.getPosition());
        }

        @Override
        public void onTabUnselected(VjiaTabLayout.Tab tab) {
            // No-op
        }

        @Override
        public void onTabReselected(VjiaTabLayout.Tab tab) {
            // No-op
        }
    }
    ...
    private class AdapterChangeListener implements ViewPager.OnAdapterChangeListener {
        ...
        public void onAdapterChanged(
            ...
            if (VjiaTabLayout.this.viewPager == viewPager) {
